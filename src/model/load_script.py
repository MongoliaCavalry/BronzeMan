...
def load_script_file(folder_path: str, script_type: list=["py", "sh", "shell"]) -> list:
    ...
    try:
        ...
        for filename in os.listdir(folder_path):
            ...
            if os.path.isfile(temp_path):
                if temp_path.endswith(tuple(script_type)):
                    ...
                    print_log(msg=f"find file: {temp_path}", level="DEBUG")
                else:
                    print_log(msg=f"skip file: {temp_path}", level="DEBUG")
            else:
                print_log(msg=f"open fodler: {temp_path}", level="DEBUG")
        ...
        if sum == 0:
            print_log(msg="no file found", level="WARNING")
    except Exception:
